...
if __name__ == "__main__":
    ...
    drop_cols = pollutants + delta_cols + lag1_cols + ['datetime', 'hour']
    ...
    # 🌟 新增：既然 X_test 里删了 lag1，我们就从总表里单独提取一份 lag1 用于"阅卷"
    Y_test_lag1 = df_processed[lag1_cols].iloc[split_idx:].copy()
    Y_pred_real = pd.DataFrame(index=Y_test_real.index)

    for p in pollutants:
        # 🌟 修复 2：从我们单独提取的 Y_test_lag1 里拿数据，完美避开 KeyError
        Y_pred_real[p] = Y_test_lag1[f'{p}_lag1'].values + Y_delta_pred_df[f'{p}_delta'].values
    ...
    for p in pollutants:
        ...
        y_b = Y_test_lag1[f'{p}_lag1'].values  # 🌟 修复 3：同样改成从 Y_test_lag1 里拿
    ...
    # 🔧 修复 3：一键导出论文排版表格
    pd.DataFrame(results_data).to_csv('RF_no_yt_results.csv', index=False, encoding='utf-8-sig')
    print("✅ 指标大表已自动保存至 'RF_no_yt_results.csv'！")
