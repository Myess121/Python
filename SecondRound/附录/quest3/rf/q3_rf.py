...
# ============ 画图验证 ============
print("\n📊 正在生成 3月16日-31日 MIMO 预测对比图...")

# 🌟 修改点 1：将过滤时间改为 3月16日
df_plot = df_eval[df_eval['datetime'] >= '2026-03-16 00:00:00'].set_index('datetime')

# 图 1：AQI 趋势 (为了容纳半个月的数据，拉长画布宽度到 16)
plt.figure(figsize=(16, 5))
# 适当调小 marker 和 linewidth，让密集的线条更好看
plt.plot(df_plot.index, df_plot['True_AQI'], label='真实 AQI', color='black', marker='.', markersize=3, linewidth=1)
plt.plot(df_plot.index, df_plot['Pred_AQI'], label='RF(MIMO)预测 AQI', color='#FF5722', linestyle='--', marker='^', markersize=3, linewidth=1)
...
plt.title('基于 RF-MIMO 架构的良乡校区空气质量(AQI) 直接多步预测 (3.16 - 3.31)', fontsize=16)
plt.ylabel('AQI 指数')
plt.legend()
plt.grid(True, alpha=0.3)
# 🌟 修改点 2：自动调整 X 轴日期显示格式，防止 15 天的日期文字挤在一起
plt.gcf().autofmt_xdate()
plt.tight_layout()
plt.savefig('Q3_RF_MIMO_March16_31_AQI.png', dpi=300)

# 图 2：六项污染物矩阵 (扩大画布尺寸到 18x12)
...
plt.suptitle('3月16日-31日 六项污染物协同预测 (RF-MIMO 架构)', fontsize=18, y=1.02)
# 同样倾斜日期，防止重叠
plt.gcf().autofmt_xdate()
plt.tight_layout()
plt.savefig('Q3_RF_MIMO_March16_31_Pollutants.png', dpi=300)

print("✅ 大功告成！半个月的完整高精度预测图已生成！请查看最新图片！")